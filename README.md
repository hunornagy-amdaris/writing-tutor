...
- **Analysis:** OpenAI (`openai` SDK) → `gpt-5.4-mini`
...
| `OPENAI_MODEL` | no | `gpt-5.4-mini` | Any OpenAI chat-completions model id. |
...
2. **Project Settings → Environment Variables** — set the variables above (only `OPENAI_API_KEY` and `NEXT_PUBLIC_ELEVENLABS_AGENT_ID` are strictly required).
